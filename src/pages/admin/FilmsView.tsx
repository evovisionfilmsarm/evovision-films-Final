...
function emptyFilmInput(): ContentInput {
  return {
    slug: '',
    type: 'movie',
    programSlug: 'kinomas',
...
function filmToInput(film: ContentItem): ContentInput {
  return {
    slug: film.slug,
    type: film.type,
    programSlug: film.programSlug,
    categorySlug: film.categorySlug,
    coverImage: film.coverImage,
    trailerUrl: film.trailerUrl ?? '',
    telegramLink: film.telegramLink ?? '',
    year: film.year,
    genre: film.genre ?? '',
...
export function FilmsView() {
  ...
  const [form, setForm] = useState<ContentInput>(emptyFilmInput());
  ...
  const loadFilms = async () => {
    ...
    setFilms(
      all.filter(
        (c) =>
          c.programSlug === 'kinomas' &&
          (c.type === 'movie' || c.type === 'episode')
      )
    );
    setLoading(false);
  };
  ...
  const openCreate = () => {
    setEditing(null);
    setForm(emptyFilmInput());
    setActiveLang('hy');
    setFormError(null);
    setShowForm(true);
  };

  const openEdit = (film: ContentItem) => {
    setEditing(film);
    setForm(filmToInput(film));
    setActiveLang('hy');
    setFormError(null);
    setShowForm(true);
  };
  ...
  const handleSave = async (publish: boolean) => {
    ...
    const input: ContentInput = {
      ...form,
      isPublished: publish,
      type: form.categorySlug === 'series' ? 'episode' : 'movie',
      programSlug: 'kinomas',
    };
  ...
  const handleUpload = async (file: File) => {
    setUploading(true);
    const url = await uploadImage(file, 'films');
    setUploading(false);
    if (url) {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="mt-1 text-sm text-white/50">{t('admin.totalFilms')}: {films.length}</p>
        </div>
        <button
          onClick={openCreate}
        ...
        <div className="rounded-xl border border-white/10 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {films.map((film) => {
                  const tr = film.translations[lang] ?? film.translations.en;
                  return (
                    <tr key={film.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-3">
                        <div className="w-12 h-16 rounded-md overflow-hidden bg-zinc-950 flex-shrink-0">
                          {film.coverImage ? (
                            <img src={film.coverImage} alt="" className="w-full h-full object-cover" />
                          ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          {film.featured && <Star className="w-3.5 h-3.5 text-red-400 fill-red-400 flex-shrink-0" />}
                          <span className="text-sm text-white font-medium">{tr.title}</span>
                        </div>
                        <span className="text-xs text-white/40">/{film.slug}</span>
                      </td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="text-sm text-white/60">{film.year ?? '—'}</span>
                      </td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="text-sm text-white/60">{film.genre ?? '—'}</span>
                      </td>
                      <td className="px-4 py-3">
                        {film.isPublished ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-zinc-900 border border-white/10 rounded-lg text-white text-sm focus:border-red-600/50 focus:outline-none transition-colors"
                  >
                    <option value="films">Ֆիլմ</option>
                    <option value="series">Սերիալ</option>
                  </select>
                </div>
