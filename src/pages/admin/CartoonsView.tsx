...
import { Film, Plus, Pencil, Trash2, X, Upload, Loader as Loader2, CircleAlert as AlertCircle, CircleCheck as CheckCircle2, Star, Globe, Image as ImageIcon } from 'lucide-react';
...
function emptyCartoonInput(): ContentInput {
  return {
    slug: '',
    type: 'movie',
...
function cartoonToInput(cartoon: ContentItem): ContentInput {
  return {
    slug: cartoon.slug,
    type: cartoon.type,
    programSlug: cartoon.programSlug,
    categorySlug: cartoon.categorySlug,
    coverImage: cartoon.coverImage,
    trailerUrl: cartoon.trailerUrl ?? '',
    telegramLink: cartoon.telegramLink ?? '',
    year: cartoon.year,
    genre: cartoon.genre ?? '',
    duration: cartoon.duration ?? '',
    difficulty: cartoon.difficulty,
    isPremium: cartoon.isPremium,
    isPublished: cartoon.isPublished,
    featured: cartoon.featured,
    sponsorId: cartoon.sponsorId,
    translations: { ...cartoon.translations },
  };
}

type Status = 'idle' | 'saving' | 'deleting';

export function CartoonsView() {
  ...
  const [form, setForm] = useState<ContentInput>(emptyCartoonInput());
  ...
  const loadCartoons = async () => {
    ...
    setCartoons(
      all.filter(
        (c: ContentItem) =>
          c.programSlug === 'cartoons' && c.type === 'movie'
      )
    );
    setLoading(false);
  };
  ...
  const openCreate = () => {
    setEditing(null);
    setForm(emptyCartoonInput());
    setActiveLang('hy');
    setFormError(null);
    setShowForm(true);
  };

  const openEdit = (cartoon: ContentItem) => {
    setEditing(cartoon);
    setForm(cartoonToInput(cartoon));
    setActiveLang('hy');
    setFormError(null);
    setShowForm(true);
  };
  ...
  const handleSave = async (publish: boolean) => {
    ...
    console.log('Saving cartoon with payload:', input);
    const result = editing
      ? await updateContent(editing.id, input)
      : await createContent(input);

    if (result.error) {
      console.error('Error saving cartoon:', result.error);
    ...
    setStatus('idle');
    setShowForm(false);
    setSuccessMsg(editing ? t('admin.saved') : t('admin.saved'));
    await loadCartoons();
  ...
  const handleUpload = async (file: File) => {
    setUploading(true);
    const url = await uploadImage(file, 'cartoons');
    setUploading(false);
    if (url) {
      setForm((f: ContentInput) => ({ ...f, coverImage: url }));
    } else {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="mt-1 text-sm text-white/50">{t('admin.totalCartoons')}: {cartoons.length}</p>
        </div>
        <button
          onClick={openCreate}
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-zinc-900/60 border-b border-white/10">
                  ...
                  <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-white/40 font-semibold hidden md:table-cell">{t('admin.year')}</th>
                  <th className="text-left px-4 py-3 text-xs uppercase tracking-wider text-white/40 font-semibold hidden md:table-cell">{t('admin.genre')}</th>
              ...
              <tbody>
                {cartoons.map((cartoon) => {
                  const tr = cartoon.translations[lang] ?? cartoon.translations.en;
                  return (
                    <tr key={cartoon.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-3">
                        <div className="w-12 h-16 rounded-md overflow-hidden bg-zinc-950 flex-shrink-0">
                          {cartoon.coverImage ? (
                            <img src={cartoon.coverImage} alt="" className="w-full h-full object-cover" />
                          ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          {cartoon.featured && <Star className="w-3.5 h-3.5 text-red-400 fill-red-400 flex-shrink-0" />}
                          <span className="text-sm text-white font-medium">{tr.title}</span>
                        </div>
                        <span className="text-xs text-white/40">/{cartoon.slug}</span>
                      </td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="text-sm text-white/60">{cartoon.year ?? '—'}</span>
                      </td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="text-sm text-white/60">{cartoon.genre ?? '—'}</span>
                      </td>
                      <td className="px-4 py-3">
                        {cartoon.isPublished ? (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            onClick={() => setDeleteTarget(cartoon)}
          ...
          <div className="w-full max-w-3xl my-8 rounded-2xl border border-white/10 bg-zinc-950 shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
              <h2 className="text-lg font-bold text-white">
                {editing ? t('admin.editFilm') : t('admin.newFilm')}
              </h2>
              <button
                onClick={closeForm}
                className="p-2 rounded-lg text-white/50 hover:text-white hover:bg-white/5 transition-colors"
            ...
            <div className="px-6 py-5 space-y-6 max-h-[calc(100vh-12rem)] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={form.slug}
                      onChange={(e) => setForm((f: ContentInput) => ({ ...f, slug: e.target.value }))}
                      placeholder="the-red-room"
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={form.year ?? ''}
                      onChange={(e) => setForm((f: ContentInput) => ({ ...f, year: e.target.value ? Number(e.target.value) : undefined }))}
                      className="w-full px-3.5 py-2.5 bg-zinc-900 border border-white/10 rounded-lg text-white text-sm focus:border-red-600/50 focus:outline-none transition-colors"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={form.categorySlug ?? 'cartoons'}
                    onChange={(e) =>
                      setForm((f: ContentInput) => ({
                        ...f,
                    ...
                    className="w-full px-3.5 py-2.5 bg-zinc-900 border border-white/10 rounded-lg text-white text-sm focus:border-red-600/50 focus:outline-none transition-colors"
                  >
                    <option value="cartoons">Ֆիլմ</option>
                  </select>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={form.genre ?? ''}
                      onChange={(e) => setForm((f: ContentInput) => ({ ...f, genre: e.target.value }))}
                      className="w-full px-3.5 py-2.5 bg-zinc-900 border border-white/10 rounded-lg text-white text-sm focus:border-red-600/50 focus:outline-none transition-colors"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      value={form.duration ?? ''}
                      onChange={(e) => setForm((f: ContentInput) => ({ ...f, duration: e.target.value }))}
                      placeholder="1h 58m"
                      className="w-full px-3.5 py-2.5 bg-zinc-900 border border-white/10 rounded-lg text-white text-sm focus:border-red-600/50 focus:outline-none transition-colors"
                    />
                  </div>
  ...
  <input
    ...
    onChange={(e) => setForm((f: ContentInput) => ({ ...f, trailerUrl: e.target.value }))}
  ...
  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm((f: ContentInput) => ({ ...f, telegramLink: e.target.value }))}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <input
                        ...
                        onChange={(e) => setForm((f: ContentInput) => ({ ...f, coverImage: e.target.value }))}
                        placeholder="https://..."
                        className="w-full px-3.5 py-2.5 bg-zinc-900 border border-white/10 rounded-lg text-white text-sm focus:border-red-600/50 focus:outline-none transition-colors"
                      />
                    </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setForm((f: ContentInput) => ({ ...f, sponsorId: e.target.value || undefined }))}
                    className="w-full px-3.5 py-2.5 bg-zinc-900 border border-white/10 rounded-lg text-white text-sm focus:border-red-600/50 focus:outline-none transition-colors"
                  >
                ...
                <div className="flex flex-wrap gap-4">
                  <label className="flex items-center gap-2.5 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setForm((f: ContentInput) => ({ ...f, featured: e.target.checked }))}
                  ...
                  <label className="flex items-center gap-2.5 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setForm((f: ContentInput) => ({ ...f, isPremium: e.target.checked }))}
                      className="w-4 h-4 rounded border-white/20 bg-zinc-900 text-red-600 focus:ring-red-600/50"
                    />
